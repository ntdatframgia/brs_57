...
@section('content')
    <div class="box-header">
      <h3 class="box-title">{{ trans('messages.list_book') }}</h3>
    ...
    <div class="box-body table-responsive no-padding">
      <table class="table table-hover">
        <tbody><tr>
          ...
          <th>{{ trans('messages.book_description') }}</th>
          <th>{{ trans('messages.number_of_page') }}</th>
          <th>{{ trans('messages.book_category') }}</th>
        ...
        <tr>
          <td>{{ $book->id }}</td>
          <td><img src=" {{ $book->img }}" class="imagess" ></img></td>
          <td>{{ $book->title }}</td>
          <td> {{ $book->author }} </td>
          ...
          <td>   {{ Form::open(array('url' => route('book.destroy', $book->id))) }}
